<template>
  <v-container fluid>
    <v-row class="text-center">
      <titulo-bar title="Alterar Funcionario" icon="mdi-account"></titulo-bar>
    </v-row>
...
export default defineComponent({
  ...
  async mounted() {
    await this.axios
      .get(`http://localhost:8080/funcionario/${this.id}`)
  ...
  methods: {
    ...
    async submitForm() {
      await this.axios
        .put(`http://localhost:8080/funcionario/${this.id}`, this.funcionario)
        .then((response) => {
